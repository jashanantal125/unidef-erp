...
frappe.listview_settings['Application'] = {
	...
	onload: function(listview) {
		// Add quick filter buttons for All, B2B, and B2C
		const filter_area = listview.filter_area;
		
		// Create filter buttons container
		const filter_buttons = $(`
			<div class="application-type-filters" style="margin-bottom: 10px; padding: 10px; background: #f8f9fa; border-radius: 4px;">
		...
		filter_buttons.on('click', '.filter-btn', function() {
			...
			if (filter_type === 'all') {
				...
				listview.filter_area.add([[listview.doctype, 'application_type', '=', filter_type]]);
			}
		...
		const has_application_type_filter = current_filters.some(f => f[0] === 'Application' && f[1] === 'application_type');
		
		if (has_application_type_filter) {
			const application_type_value = current_filters.find(f => f[0] === 'Application' && f[1] === 'application_type')[2];
			if (application_type_value === 'B2B') {
				filter_buttons.find('[data-filter="B2B"]').removeClass('btn-default').addClass('btn-primary');
			} else if (application_type_value === 'B2C') {
